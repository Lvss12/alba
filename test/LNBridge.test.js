...
describe("ALBA", function(account) {
    ...
    describe("Channel basic lifecycle", function () {
        it("Open, update, and close channel by signatures", async function () {
            // recall identity P
            const entropyP = Buffer.from('ciaociaociaociaociaociaociaociaociaociaociaociaociaociaociaociaociaociaociaociaociaociaociaociaociaociaociaociaociaociaociaociao', 'utf-8');
            ...
            const openMsg = sha256(
                ethers.utils.solidityPack(
                    ["address", "uint256", "uint256", "uint256", "bytes32", "string"],
                    [ALBA.address, 0, balP0, balV0, rKey0, "openChannel"]
                )
            );
            const sigOpenP = EthCrypto.sign(identityP.privateKey, openMsg);
            const sigOpenV = EthCrypto.sign(identityV.privateKey, openMsg);

            await expect(ALBA.openChannel(balP0, balV0, rKey0, sigOpenP, sigOpenV))
                .to.emit(ALBA, "channelOpened")
                .withArgs(balP0, balV0);
            ...
            const updMsg = sha256(
                ethers.utils.solidityPack(
                    ["address", "uint256", "uint256", "uint256", "bytes32", "string"],
                    [ALBA.address, seq1, balP1, balV1, rKey1, "updateChannel"]
                )
            );
            const sigUpdP = EthCrypto.sign(identityP.privateKey, updMsg);
            const sigUpdV = EthCrypto.sign(identityV.privateKey, updMsg);

            await expect(ALBA.updateChannel(seq1, balP1, balV1, rKey1, sigUpdP, sigUpdV))
                .to.emit(ALBA, "channelUpdated")
                .withArgs(seq1, balP1, balV1);

            const stateHash = sha256(
                ethers.utils.solidityPack(
                    ["uint256", "uint256", "uint256", "bytes32"],
                    [seq1, balP1, balV1, rKey1]
                )
            );
            const closeMsg = sha256(
                ethers.utils.solidityPack(
                    ["address", "bytes32", "string"],
                    [ALBA.address, stateHash, "closeChannel"]
                )
            );
            const sigCloseP = EthCrypto.sign(identityP.privateKey, closeMsg);
            const sigCloseV = EthCrypto.sign(identityV.privateKey, closeMsg);

            await expect(ALBA.closeChannel(seq1, balP1, balV1, rKey1, sigCloseP, sigCloseV))
                .to.emit(ALBA, "channelClosed")
                .withArgs(seq1, balP1, balV1);
        });
    });
